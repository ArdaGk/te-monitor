...
api_key = 'fDewKPKkdj9F7gIljXMKjE2zauAmVPmMnQMapatvylvtcIC0hEXu8Qab7ex5fTjO'
api_secret = 'SnO0WP0xzNsPYEFXsUnd2oqL558z6pEwOo6phsrMgJZiSoGHfDK6qmDufzyihpMG'
addr = 'http://35.242.188.25/'
zulu = datetime.now(tz=timezone.utc).timestamp()
...
def graph(symbol, date, time='00:00:00'):
  ...
  !gsutil cp gs: // binance-te/{day}.json data.json
  data = load('data')

  traces = list(data.values())[-1][symbol]['traces']
  traces = {week: list(traces.values())}

  client = Spot(api_key, api_secret, show_limit_usage=True)
  trades = [(datetime.fromtimestamp(trd['time'] / 1000), float(trd['price']),
             'BUY' if trd['isBuyer'] else 'SELL') for trd in client.my_trades('LINKUSDT')['data']]
...
def get_klines(symbol, interval, start):
  ...
  klines = client.klines(symbol, interval, startTime=start *
                         1000, endTime=(start + 86400) * 1000)
  output = []
...
def plot(df, traces, trades, interval, even=False, symbol='symbol: unknown'):
  ...
  fig.add_trace(go.Scatter(x=traced['time'], y=traced['T1'], line=dict(
      color='red', dash='dot', width=1)))
  fig.add_trace(go.Scatter(x=traced['time'], y=traced['T2'], line=dict(
      color='red', dash='dot', width=1)))
  fig.add_trace(go.Scatter(x=traced['time'], y=traced['T3'], line=dict(
      color='green', dash='dot', width=1)))
  fig.add_trace(go.Scatter(x=traced['time'], y=traced['T4'], line=dict(
      color='green', dash='dot', width=1)))
  ...
  fig.add_trace(go.Scatter(x=[trd[0] for trd in buys], y=[trd[1] for trd in buys],
                mode='markers', marker_size=8, marker_line_width=2, marker_color='red', name='Bought'))
  fig.add_trace(go.Scatter(x=[trd[0] for trd in sells], y=[trd[1] for trd in sells],
                mode='markers', marker_size=8, marker_line_width=2, marker_color='green', name='Sold'))
  fig.update_layout(title_text=symbol, title_x=0.5, paper_bgcolor=bg,
                    plot_bgcolor='rgba(255,0,0,0)', font_color="rgba(184,184,184,1)")
...
def graphs():
  ...
  for symbol in symbols:
    try:
      ...
      plot(df, traces, trades, '1d', even=(count % 2 == 0), symbol=symbol)
      count += 1
    except:
      pass
