...
def worth():
  ...
  prices = {p['symbol']: float(p['price']) for p in client.ticker_price()}
  prices['USDT'] = 1
  total = 0
  for bal in client.account()['balances']:
    try:
      quote = '' if bal['asset'] == 'USDT' else "USDT"
      free = float(bal['free']) * prices[bal['asset'] + quote]
      locked = float(bal['locked']) * prices[bal['asset'] + quote]
      full = free + locked
      total += full
      if round(full,2) != 0:
        line = f"{bal['asset']}\t${round(full,2)}"
        print(line)
    except:
      continue
  print('================')
